...
async def action_push_submit_button(user_input, state):
    ...
    if state["gen_precondition"]:
        response = await run_agent(
            current_agent, user_input, current_state, current_config
        )
        ...
        yield (
            ...
            gr.update(value=state["md_value"]),
            gr.update(
                value=state["files_visible"], visible=state["file_visible_status"]
            ),
            "",
        )
...
async def action_click_next_button(state):
    ...
    try:
        ...
        if curent_status == "CO":
            ai_message = {
                "role": "assistant",
                "content": "Введите название репозитория",
            }
            state["messages"].append(ai_message)
        button_params = {"value": f"{settings.BUTTON_STATUS[curent_status]}"}
        header_params = {
